...
def accueil():
    ...
    image_path = os.path.join(os.path.dirname(__file__), "../img", "rechauffement-climatique-petrole-usine.webp")
    ...
    st.markdown('''
                <p style="text-align: justify;">Dans le cadre de notre formation Data Analyst en Bootcamp chez Datascientest, nous avons travaillé sur un projet « fil rouge » nous permettant d’aborder toutes les étapes de développement d’une solution data. 
    Nous avons été sélectionnés pour le projet « Température terrestre » avec comme objectif de constater le réchauffement 
    (et le dérèglement) climatique global à l’échelle de la planète sur les derniers siècles et dernières décennies.   
    Ce projet nous a permis d’appliquer les compétences apprises au cours de la formation, de pouvoir travailler en équipe et 
    d’avoir la capacité d’apprendre sur des problématiques métier.</p>''', unsafe_allow_html=True)
    st.write("")
    content = """
...
# 
    ...
    if st.button("Afficher les définitions et explications"):
        ...
        image_path = os.path.join(os.path.dirname(__file__), "../img", "composantes_climatique.png")
        img = Image.open(image_path)
        st.image(img)
#   
    st.markdown('''
    ...
    if st.button("Afficher les éléments de Gestion du projet"):
        ...
        image_path = os.path.join(os.path.dirname(__file__), "../img", "Diagramme de Gantt_page_1.jpg")
        img = Image.open(image_path)
        st.image(img, use_column_width=True)
        
        image_path = os.path.join(os.path.dirname(__file__), "../img", "Diagramme de Gantt_page_2.jpg")
        img = Image.open(image_path)
        st.image(img, use_column_width=True)
